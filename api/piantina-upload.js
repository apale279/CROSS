...
async function uploadBufferToCloudinary(buffer, mime, tenantId) {
  ...
  form.append('file', new Blob([buffer], { type: mime || 'image/png' }), `piantina.${ext}`);
  form.append('folder', `cross/piantine/${tenantId}`);
  form.append('tags', 'cross,piantina');
  ...
  try {
    ...
    if (!cfg) {
      return res.status(503).json({
        error:
          'Configura CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY e CLOUDINARY_API_SECRET su Vercel.',
      });
    }
    ...
    if (buffer.length > 8 * 1024 * 1024) {
      return res.status(400).json({ error: 'File troppo grande (max 8 MB)' });
    }

    const url = await uploadBufferToCloudinary(buffer, mime, tenantId);
    return res.status(200).json({ ok: true, url });
  } catch (err) {
    const status = err.status ?? 500;
    if (status >= 500) console.error('[piantina-upload]', err);
